...
LevelOne::LevelOne(sf::RenderWindow& game_window) : Level(game_window) {}

bool LevelOne::init(Player* _player, UI* _ui)
{
  init_setup_blocks();
...
void LevelOne::init_setup_blocks()
{
  ...
  for (int i = 0; i < WIDTH; i++)
  {
    for (int j = 0; j < HEIGHT; j += 18)
    {
      world[i * HEIGHT + (j + 15)]->init(
        grass_top_textures[1], 0 + i * block_width, 0 + (j + 15) * block_width);
...
void LevelOne::init_setup_platforms()
{
  ...
  for (int i = 0; i < 8; i++)
  {
    generatePlatform(platform_x[i], platform_y[i], platform_width[i], grass_platform_textures,gems[i]);
  }

  generateBlock(10, 3, 0);
  generateBlock(17, 3, 0);

  generateBox(38, 11, 3, 4, grass_top_textures, true);

  // Creates even size staircases e.g. if 5 wide, needs to be 5 high
  generateStairs(26, 12, 3, 3, grass_top_textures[0], 0, true);
  generateStairs(32, 12, 3, 3, grass_top_textures[2], 0, false);
}

void LevelOne::init_setup_enemies()
{
